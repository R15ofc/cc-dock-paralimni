...
return {
  run = function(ctx)
    ...
    local update_begin = ctx.update_service.beginCheck()
    local update_status = ctx.update_service.status()
    check("update service", update_status.ok and update_begin.ok and update_status.data.status == "checking" and update_status.data.progress == 0)
    ...
    local studio_mode = ctx.studio_service.setMode("script")
    local studio_tool = ctx.studio_service.setTool("add", "shape")
    local studio_add = ctx.studio_service.addComponent("input", 44, 32)
    ...
    local studio_line = ctx.studio_service.setScriptLine(1, "app.name = \"Selftest Studio App\"")
    local studio_insert_line = ctx.studio_service.insertScriptLine(2, "app.icon = \"placeholder\"")
    local studio_delete_line = ctx.studio_service.deleteScriptLine(2)
    local studio_icon = ctx.studio_service.setIcon("placeholder")
    local studio_scroll_panel = ctx.studio_service.scrollPanel(1, 40)
    local studio_duplicate = ctx.studio_service.duplicateSelected()
    local studio_nudge = ctx.studio_service.nudgeSelected(2, 1)
    local studio_delete = ctx.studio_service.deleteSelected()
    local studio_save = ctx.studio_service.save()
    local studio_projects = ctx.studio_service.listProjects()
    local studio_open = ctx.studio_service.openProject("user.selftest_studio_app")
    local studio_clicker = ctx.studio_service.loadExample("clicker")
    ...
    check("studio service", studio_new.ok and studio_mode.ok and studio_tool.ok and studio_add.ok and studio_add.data.components[studio_add.data.selected].x == 44 and studio_move.ok and studio_resize.ok and studio_source.ok and studio_line.ok and studio_insert_line.ok and studio_delete_line.ok and studio_icon.ok and studio_scroll_panel.ok and studio_duplicate.ok and studio_nudge.ok and studio_delete.ok and studio_save.ok and studio_projects.ok and #studio_projects.data >= 1 and studio_open.ok and studio_clicker.ok and studio_clicker.data.components[2].binding == "storage.clicks" and studio_example.ok and studio_diagnostics.ok and studio_diagnostics.data.count == 0 and studio_export.ok and fs.exists(studio_export.data.manifest) and fs.exists(studio_export.data.ui) and studio_export.data.path:match("%.app$") ~= nil)
