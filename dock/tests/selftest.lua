...
return {
  run = function(ctx)
    ...
    local drag_source = ctx.explorer_service.createFile(explorer_id, "drag.txt")
    local drag_target = ctx.explorer_service.movePathTo(explorer_id, drag_source.data, folder.data)
    check("explorer drag move", drag_source.ok and drag_target.ok and fs.exists(drag_target.data) and fs.getDir(drag_target.data) == folder.data)
    ...
    local studio_resize = ctx.studio_service.resizeComponent(1, 4, 4)
    local studio_source = ctx.studio_service.sourceCode()
    local studio_line = ctx.studio_service.setScriptLine(1, "# DockScript Selftest")
    ...
    check("studio service", studio_new.ok and studio_add.ok and studio_move.ok and studio_resize.ok and studio_source.ok and studio_line.ok and studio_icon.ok and studio_example.ok and studio_export.ok and fs.exists(studio_export.data.manifest) and fs.exists(studio_export.data.ui) and studio_export.data.path:match("%.app$") ~= nil)
