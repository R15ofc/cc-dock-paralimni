...
local explorer_service = require("dock.system.explorer_service")
local update_service = require("dock.system.update_service")
...
function M.new()
  ...
  ctx.explorer_service = explorer_service.new(ctx)
  ctx.update_service = update_service.new(ctx)
...
function M.initialize(ctx)
  ...
  seed_file(paths.join(paths.etc, "services.json"), { autostart = { "devices", "notifications", "time", "ipc", "permissions", "app_runtime", "windows", "explorer", "updates", "net" } })
  ...
  ctx.service_manager.register("explorer", ctx.explorer_service, { autostart = true })
  ctx.service_manager.register("updates", ctx.update_service, { autostart = true })
