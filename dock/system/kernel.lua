...
local cursor_service = require("dock.system.cursor_service")
local text_input_service = require("dock.system.text_input_service")
local menu_service = require("dock.system.menu_service")
local studio_service = require("dock.system.studio_service")
local update_service = require("dock.system.update_service")
local cloud_service = require("dock.system.cloud_service")
...
function M.new()
  ...
  ctx.cursor_service = cursor_service.new(ctx)
  ctx.text_input_service = text_input_service.new(ctx)
  ctx.menu_service = menu_service.new(ctx)
  ctx.studio_service = studio_service.new(ctx)
  ctx.update_service = update_service.new(ctx)
  ctx.cloud_service = cloud_service.new(ctx)
...
function M.initialize(ctx)
  ...
  seed_file(paths.join(paths.etc, "services.json"), { autostart = { "devices", "notifications", "time", "ipc", "permissions", "app_runtime", "windows", "explorer", "cursor", "text_input", "menus", "studio", "updates", "cloud", "net" } })
  ...
  seed_file(paths.join(paths.etc, "permissions.json"), { permissions = { "fs.read", "fs.write", "fs.delete", "network.rednet", "network.http", "peripheral.access", "settings.read", "settings.write", "process.spawn", "ipc.message", "notification.send", "storage.app", "window.manage" } })
  ctx.user_service.load()
  ...
  ctx.service_manager.register("cursor", ctx.cursor_service, { autostart = true })
  ctx.service_manager.register("text_input", ctx.text_input_service, { autostart = true })
  ctx.service_manager.register("menus", ctx.menu_service, { autostart = true })
  ctx.service_manager.register("studio", ctx.studio_service, { autostart = true })
  ctx.service_manager.register("updates", ctx.update_service, { autostart = true })
  ctx.service_manager.register("cloud", ctx.cloud_service, { autostart = true })
